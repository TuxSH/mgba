...
static void _drawTex(struct mCore* core, bool faded, bool both) {
	...
	if (filterMode < FM_LINEAR_1x || filterMode > FM_LINEAR_3x) {
		// Other filtering modes are not supported on 3DS
...
static void _incrementFilterMode(struct mGUIRunner* runner) {
	UNUSED(runner);
	filterMode = (filterMode + 1) % FM_MAX;
	if (filterMode == FM_NEAREST) ++filterMode;
...
int main(int argc, char* argv[]) {
	...
	struct mGUIRunner runner = {
		...
		.incrementFilterMode = _incrementFilterMode,
	...
	_map3DSKey(&runner.params.keyMap, KEY_ZR, mGUI_INPUT_FILTER_MODE);
